...
//Code Here
let myStrongest  = monstersInYourPocket.filter(el => el["CP"]>200)
...
let sum = 0;
let ordersTotal = orders.map(function(el) {
  let totalPrice = el["price"]*(el["tax"]+1);
  sum += totalPrice;
})
...
let bobsTotal = purchases.reduce((total, el) => {
  if(el.owner==='Bob') {
